...
import textfighter.main.Casino;
...
public class Achievements {
	...
	private static void checkTextFighterMaster() {
		if (moneyMaker && enemySlayer && firstKill && timeForAnUpgrade && YAYPOWER && awwYouCareAboutMe && slayer && nobodysPerfect
				&& makingMoney && gamblingAddiction && level2Fighter && level3Fighter && level4Fighter && level5Fighter && level6Fighter 
				&& level7Fighter && level8Fighter && level9Fighter && level10Fighter && honestPlayer) {
			int temp = 0;
			for (int i = 0; i <= arrayEnemy.size(); i++) {
				if (arrayKilled.get(i)) {
					temp++;
			...
			if (temp < arrayEnemy.size()) {
				...
				get("Text-Fighter Master\n Congratulations, you have gotten every achivements in Text Fighter! You've been awarded 2500 coins.");
				Coins.set(2500, true);
			}
		}
	}
